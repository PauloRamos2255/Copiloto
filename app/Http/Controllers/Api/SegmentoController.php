   ...
   public function guardar(Request $request)
{
    ...
    foreach ($zonas as $z) {
        ...
        $zonasData[] = [
            ...
            'nombre'      => $z['n'] ?? $z['nombre'] ?? '',
            'color'       => $z['c'] ?? '#FFFFFF',
            'cordenadas'  => !empty($z['p']) && is_array($z['p']) ? json_encode($z['p'], JSON_UNESCAPED_UNICODE) : null,
            'bounds'      => !empty($z['b']) && is_array($z['b']) ? json_encode($z['b'], JSON_UNESCAPED_UNICODE) : null,
            'updated_at'  => now(),
            // created_at solo si es un nuevo registro
            'created_at'  => now(),
        ];
    }
